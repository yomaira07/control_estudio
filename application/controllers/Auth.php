...
redirect(base_url());
	...
	//	$usuario = $this->input->post("username");
		...
		if($data->estado==1 and ($data->rol_id==5 or $data->rol_id==8)){
				...
				if($data->email==$this->input->post("correo")){
					...
					if($this->Usuarios_model->update_usuario($id_usuario,$data_up)){
						...
						//var_dump($configGmail);
							...
							<h2><br> Usuario: '.$cedula .'<br> Contraseña:'.$contrasena.' <br>Acceso como: '.$rol.'</h2>										
							</p>
							<p>
							Importante:
						...
						{ */
							...
							$this->session->set_flashdata("info","Verificación exitosa.".$msg);
			...
			$this->session->set_flashdata("warning","Usuario y correo electrónico Inactivo. Para recuperar su acceso como estudiante regular o egresado
			 debe hacer clic en el botón SOLO ESTUDIANTE REGULAR O EGRESADO RECUPERAR ACCESO");
			redirect(base_url());
		}elseif( $data->rol_id==7){
			$this->session->set_flashdata("error","El Usuario y Correo electrónico corresponde al Proceso de Seleción de Aspirantes.En caso de dudas por favor enviar un email a <b>tecnologia@enf.edu.ve </b> para verificar su acceso.
			<b>NOTA</b>: Indicar en la descripción del correo su Cédula de identidad, Nombres y Apellidos y pequeña reseña de la falla.");
			redirect(base_url());
		}else{
			$this->session->set_flashdata("error","El Usuario y Correo electrónico debe ser revisado.Por favor comunicarse con <b>tecnologia@enf.edu.ve </b> para verificar su acceso.
			<b>NOTA</b>: Indicar en la descripción del correo su Cédula de identidad, Nombres y Apellidos y pequeña reseña de la falla.");
			redirect(base_url());
		}
	}
 public function enviar_correo()// envia la clave si olvido contraseña
	{
		//preparamos los datos
		...
		if(!$this->Usuarios_model->buscar_correo($correo)){
					if($this->Usuarios_model->update_usuario($id_usuario,$data_up)){
						$this->session->set_flashdata("success","El correo electrónico fue actiualizado con éxito...");
						redirect(base_url()."admin/usuario/cambiar_correo/");
					}else{
						$this->session->set_flashdata("error","No se pudo guardar la información");
						redirect(base_url()."admin/usuario/cambiar_correo/");
					}
		}else{						
					$this->session->set_flashdata("warning","No se pudo actualizar el correo electrónico sugerido. Posiblemente el correo se encuentra registrado con otro usuario, intente con un correo distinto.");
					redirect(base_url()."admin/usuario/cambiar_correo/");
		}
