...
class Dashboard07 extends CI_Controller { // controlador NOTAS ACADEMICAS (RECORD Academico)
...
public function buscar_cedula(){
		...
		$data = array('periodo'=>$this->Periodo_model->PeriodoActivoNotas()		);
 		
		$this->load->view('layouts/header');
		$this->load->view('layouts/sidebar');
		$this->load->view('supervisor/plan_clases/buscar_notas',$data);
...
public function index()
{
	...
	$data = array(
	'listado' => $this->Notas_academica_model->getBuscarnotas_todas($cedula),		
	'datos_alumno'	=> $this->Alumno_model->buscar_estudiante_cedula1($cedula)
	);
//	var_dump($data);
	$this->load->view('layouts/header');
	$this->load->view('layouts/sidebar');
	$this->load->view('supervisor/plan_clases/notas_cargadas_sist',$data);
	$this->load->view('layouts/footer');
}
